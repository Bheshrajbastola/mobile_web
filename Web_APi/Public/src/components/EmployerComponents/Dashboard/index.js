...
//for pagination
  ...
  // Step 1: Add state variable for current page
  const [currentPage, setCurrentPage] = useState(1);

  // Step 2: Separate array to hold jobs for the current page
  const jobsPerPage = 3;
  const indexOfLastJob = currentPage * jobsPerPage;
  const indexOfFirstJob = indexOfLastJob - jobsPerPage;
  const currentJobs = jobInfo.slice(indexOfFirstJob, indexOfLastJob);
...
const handleDeleteConfirmation = (jobId) => {
  ...
  const handleDeleteJob = async (jobId) => {
    try {
      ...
      const response = await axios.delete(deleteJobRoute(jobId), config);
  
      if (response.status === 200) {
        // Job deleted successfully, you can update the job list or take any other necessary action
        getCompanyJob(); // Assuming getCompanyJob is a function that fetches the company's jobs again after deletion
        toast.success("Job deleted successfully", toastOptions);
      } else {
        toast.error("Failed to delete job", toastOptions);
      }
    } catch (error) {
      console.error(error);
      toast.error("An error occurred while deleting the job", toastOptions);
    }
  };
        ...
        <JobsContainerRow className="row">
          {
            // Map through jobs and display them in the dashboard if not loading
                  ...
                  !loading && jobInfo.map((jobDetail) => (
    <JobContainer key={jobDetail.job._id} className="col-12">
          ...
          href="#"
          className="dropdown-item px-4 py-2"
          onClick={() => handleDeleteConfirmation(jobDetail.job._id)}
        >
          Delete
        </DropLink>
        ...
        <div className="d-flex justify-content-center mt-3">
          ...
          <button
            ...
            className="btn btn-outline-primary mx-2"
            onClick={() =>
              setCurrentPage((prev) => Math.min(prev + 1, Math.ceil(jobInfo.length / jobsPerPage)))
            }
            disabled={currentPage === Math.ceil(jobInfo.length / jobsPerPage)}
          >
            Next Page
          </button>
        </div>
